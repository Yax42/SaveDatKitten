IconAssets = group{quality=9}
Icon = image{"resources/icon.png"}

GameAssets = group{quality=9}
RockTile = image{"resources/rock.png"}
SandTile = image{"resources/sand.png"}
GrassTile = image{"resources/grass.png"}
Pikachu = image{"resources/pikachu.png", pinned=true, width=32, height=32}
Toad = image{"resources/toad.png", pinned=true, width=32, height=32}
Cat = image{"resources/cat.png", pinned=true, width=32, height=32}
Plants = image{"resources/plants.png", pinned=true, width=64, height=64}
Empty = image{"resources/empty.png", pinned=true, width=8, height=8}
Paw = image{"resources/paw.png", pinned=true, width=32, height=32}
Shroom = image{"resources/shroom.png", pinned=true, width=32, height=32}
Flash = image{"resources/flash.png", pinned=true, width=32, height=32}
UpDown = image{"resources/UpDown.png", pinned=true, width=128, height=32}
LeftRight = image{"resources/LeftRight.png", pinned=true, width=32, height=128}
SuccessFail = image{"resources/SuccessFail.png", pinned=true, width=64, height=64}
Perdre = image{"resources/perdre.png", pinned=true, width=64, height=64}
VerticalWall = image{"resources/sideWall.png"}
HorizontalWall = image{"resources/topWall.png"}
BlackTile = image{"resources/black.png"}
IsHere = image{"resources/isHere.png"}